...
namespace A7.Navigation {
    ...
    export class Navigator implements A7.Navigation.INavigator {
        ...
        handleStateChange(state: Navigation.INavigateState, fnHandler: (state: Navigation.INavigateState) => void ): void {
        ...
        To(url: string, urlParams: { } = null, title: string = '', data: { } = {}): void {
            ...
            var newstate = <Navigation.INavigateState>{ Url: url, Data: data };
            if (title)
                newstate.Title = title;

            this.handleStateChange(newstate, (state: Navigation.INavigateState) => {
        ...
        Replace(url: string, urlParams: { } = null, title: string = '', data: { } = {}): void {
            url += urlParams ? $.param(urlParams) : '';
            this.handleStateChange({ Url: url, Title: title, Data: data }, (state: Navigation.INavigateState) => {
        ...
        GetState(): Navigation.INavigateState { 
            var state = this._history.getState();
            return { Url: state.url, Title: state.title, Data: state.Data } 
        }

        OnNavigate(fnHandler: (navigateState: Navigation.INavigateState) => void): void {
